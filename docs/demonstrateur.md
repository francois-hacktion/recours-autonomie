...
| Assistant vocal | `screens/AssistantVocal.tsx` | **Accueil** (Silneo se présente, demande le prénom), **cadrage** (que cherche la personne ?), puis **deux branches jouées jusqu'au bout** : « renseignement » (aide locale, l'adaptation du logement, avec **recherche locale visible** puis mise en relation) ou « labyrinthe » (4 questions, moteur, APA + ASPA, **récap des montants envoyé par mail au demandeur**, mise en relation). La mise en relation propose le canal : fiche d'appel, ou message envoyé par le service au guichet, le **numéro de rappel n'étant demandé qu'à ce moment**, avant consentement. L'assistant est nommé « Silneo » |
